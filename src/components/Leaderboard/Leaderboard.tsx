...
const Leaderboard: React.FC = () => {
  ...
  const mockData: Record<GameName, Record<"Daily" | "Weekly" | "Monthly" | "Overall", { rank: number; wallet: string; score: number }[]>> = {
    "Cricket Catch Pro": {
      Daily: [
  ...
  const [selectedGame, setSelectedGame] = useState<GameName>("Cricket Catch Pro");
  const [timeframe, setTimeframe] = useState<"Daily" | "Weekly" | "Monthly" | "Overall">("Daily");
  ...
  return (
    <div className="bg-black relative min-h-screen w-full overflow-x-hidden text-white">
      <div className="relative w-full h-[15vh] bg-black">
        <h3 className="text-white text-2xl font-semibold absolute left-[32%]">Leaderboard</h3>
        <select
      ...
      <div className="bg-black w-[103%] relative left-[-10px] h-[80vh] rounded-[30px] border-3 border-[#92FF00]">
        ...
        <div className="top3 flex justify-center items-end gap-0 p-4">
        ...
        <div>
          <table className="w-full border-collapse bg-[#565656] text-center">
            <thead>
              <tr>
                <th className="border-b text-[#92FF00] text-semibold border-gray-600 py-2">Rank</th>
            ...
            <tbody>
              {mockData[selectedGame][timeframe].slice(3).map((entry, index) => (
                <tr key={index}>
                  <td className="border-b text-[#B9B9B9]  border-gray-600 py-2">{entry.rank}</td>
                  <td className="border-b text-[#B9B9B9]  border-gray-600 py-2">{entry.wallet}</td>
                  <td className="border-b text-[#B9B9B9]  border-gray-600 py-2">{entry.score}</td>
                </tr>
              ))}
            </tbody>
          </table>
