...
const TrendingGamesCarousel: React.FC<TrendingGamesCarouselProps> = ({ trendingGames }) => {
  ...
  const nextSlide = () => {
    setCurrentIndex((prevIndex) => (prevIndex + 1) % trendingGames.length);
    resetTimeout();
  };

  const prevSlide = () => {
    setCurrentIndex((prevIndex) => prevIndex === 0 ? trendingGames.length - 1 : prevIndex - 1);
    resetTimeout();
  };
                ...
                <div className="absolute inset-0 flex flex-col justify-end p-8 z-10">                  
                  <div className="flex items-center text-lg mb-4">
                    <span className="bg-blue-600 px-3 py-1 rounded-full mr-3">
                      {game.numberofplayer} Players
                    </span>
                  </div>
                  
                  <button
                    className="bg-red-600 hover:bg-red-700 text-white rounded-full flex items-center justify-center w-48 py-3 font-bold text-lg transition-all duration-300"
                  >
